...
export async function generateRepoContext(options) {
    ...
    const [stackResult, commands, environmentVariables, importantFiles, folderTree] = await Promise.all([
        scanStack(rootDir),
...
function buildNotes(input) {
    ...
    if (input.detectedStack.length === 0) {
        notes.push("No stack was confidently detected. Consider adding README.md or standard config files.");
    }
    if (input.commandsCount === 0) {
        notes.push("No runnable commands were detected from package.json, Makefile or Docker Compose.");
    }
    if (input.envCount === 0) {
        notes.push("No environment variables were detected. Add .env.example to document required configuration.");
    }
    notes.push("This context was generated locally without an AI model. No source code contents were included.");
    return notes;
}
