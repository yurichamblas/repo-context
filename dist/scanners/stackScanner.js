...
export async function scanStack(rootDir) {
    const stack = new Set();
    ...
    for (const relativePath of signalFiles) {
        ...
        else if (base === "docker-compose.yml" ||
    ...
    await Promise.all(packageJsonPaths.map(async (relativePath) => {
    ...
    await Promise.all(pythonManifestPaths.map(async (relativePath) => {
        const content = (await readTextFile(path.join(rootDir, relativePath)))?.toLowerCase();
        if (!content)
            return;
