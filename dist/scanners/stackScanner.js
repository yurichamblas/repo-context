...
export async function scanStack(rootDir) {
    ...
    const pyproject = (await readTextFile(path.join(rootDir, "pyproject.toml")))?.toLowerCase();
    ...
    if (pyproject?.includes("flask"))
        stack.add("Flask");
    const requirements = (await readTextFile(path.join(rootDir, "requirements.txt")))?.toLowerCase();
    ...
    if ((await fileExists(path.join(rootDir, "docker-compose.yml"))) ||
